...
export const RemediationPanel: React.FC<RemediationPanelProps> = ({
    ...
    const [applyFixes, setApplyFixes] = useState<Set<string>>(new Set());
    ...
    const handleApplyFix = async (suggestion: RemediationSuggestion, suggestionId: string) => {
        setApplyFixes((prev) => new Set(prev).add(suggestionId));
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
      ...
      <div className="space-y-3">
        {suggestions.map((suggestion, index) => {
          ...
          const isApplying = applyFixes.has(suggestionId);
